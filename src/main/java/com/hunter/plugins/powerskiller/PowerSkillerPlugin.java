...
import com.hunter.plugins.HunterUtils.API.MessageUtils;
import com.hunter.plugins.HunterUtils.BreakHandler.ReflectBreakHandler;
import com.hunter.plugins.HunterUtils.API.InventoryUtil;
...
public class PowerSkillerPlugin extends Plugin {
    ...
    private List<String> copiedItems = new ArrayList<>();
    ...
    public void copyInventory() {
        ...
        // Use a Set to ensure only unique item names are saved
        Set<String> uniqueItems = new HashSet<>(InventoryUtil.getItemNames());

        copiedItems = new ArrayList<>(uniqueItems); // Convert the Set back to a List if needed

        if (copiedItems.size() > 0) {
            MessageUtils.addMessage("Copied " + copiedItems.size() + " unique items from inventory to bank", Color.GREEN);
            MessageUtils.addMessage("Items copied: " + String.join(", ", copiedItems), Color.GREEN);
            MessageUtils.addMessage("test" + copiedItems, Color.GREEN);
        } else {
            MessageUtils.addMessage("Inventory is empty", Color.RED);
        }
    }

    private void updateConfigWithCopiedItems() {
        if (copiedItems.isEmpty()) {
            return;
        }
        String newItemsToKeep = String.join(",", copiedItems);
        configManager.setConfiguration("PowerSkiller", "itemsToKeep", newItemsToKeep);
        configManager.setConfiguration("PowerSkiller", "copyInventory", false);
    }

    private void refreshItemsToKeep() {
        String updatedItemsToKeep = configManager.getConfiguration("PowerSkiller", "itemsToKeep");
        configManager.setConfiguration("PowerSkiller", "itemsToKeep", updatedItemsToKeep);
        MessageUtils.addMessage("Config refreshed with new items to keep: " + updatedItemsToKeep, Color.GREEN);
    }
}
